...
export default function IssueModal({ task, onClose, onRefresh, supabase }: Props) {
  ...
  async function addIssue() {
    ...
    const { error } = await supabase.from('issues').insert({ task_id: task.id, text: newText.trim(), memo: newMemo.trim(), done: false })
    if (error) { alert('課題の追加に失敗しました：' + error.message); return }
    setNewText(''); setNewMemo(''); setShowAdd(false)
    await onRefresh()
  }

  const done = task.issues.filter(s => s.done).length
  const total = task.issues.length
      ...
      <div style={modalStyle}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
          <div>
            ...
            <div style={{ fontSize: 12, color: '#888', marginTop: 2 }}>課題</div>
          </div>
        ...
        <div>
          <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 11, color: '#888' }}>
            <span>{done} / {total} 解決</span><span>{pct}%</span>
          </div>
          ...
          {task.issues.map(s => (
            <div key={s.id} style={subItemStyle}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px 10px' }}>
                <div onClick={() => toggleIssue(s)} style={chkStyle(s.done)}>
                ...
                <button onClick={() => deleteIssue(s.id)} style={smallDelBtnStyle}><span style={{ fontSize: 16 }}>×</span></button>
              </div>
              {s.memo && (
        ...
        {showAdd ? (
          <div style={{ border: '1px solid #ddd', borderRadius: 8, background: '#fff', overflow: 'hidden' }}>
            <input value={newText} onChange={e => setNewText(e.target.value)} placeholder="課題を入力..." style={{ width: '100%', fontSize: 16, padding: '8px 10px', border: 'none', outline: 'none', fontFamily: 'inherit' }} autoFocus />
            ...
            <div style={{ display: 'flex', gap: 8, padding: '8px 10px', borderTop: '0.5px solid #eee', background: '#fafafa' }}>
              <button onClick={addIssue} style={tinySaveBtnStyle}>追加する</button>
              <button onClick={() => setShowAdd(false)} style={tinyCancelBtnStyle}>キャンセル</button>
            </div>
          </div>
        ) : (
          <button onClick={() => setShowAdd(true)} style={addTriggerStyle}><span style={{ fontSize: 13 }}>+</span> 課題を追加</button>
        )}
